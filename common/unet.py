...
from common.modules import Linear, Conv2d, SamePad2d, Sequential
...
def get_timestep_embedding(timesteps, embed_dim: int, dtype: torch.dtype = DEFAULT_DTYPE):
    ...
    # this supports the original unet implementation used by the image-domain diffusion models.
    """
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.levels = levels
        self.downsamples = nn.ModuleDict({f"level_{i}": self.get_downsample_by_level(i) for i in range(levels)})
        ...
        self.upsamples = nn.ModuleDict({f"level_{i}": self.get_upsample_by_level(i) for i in range(levels)})
    ...
    def get_block_by_level(self, level):
        # get block by level.
        # this supports the shared training, model, and diffusion utilities used throughout the repo.
    ...
    def get_downsample_by_level(self, level):
        # get downsample by level.
        # this supports the shared training, model, and diffusion utilities used throughout the repo.
        block = self.get_block_by_level(level)
    ...
    def get_upsample_by_level(self, level):
        # get upsample by level.
        # this supports the shared training, model, and diffusion utilities used throughout the repo.
        block = self.get_block_by_level(level)
